...
FastAPI application for Vercel deployment - FULL VERSION
"""
...
# Initialize services (only if imports succeeded)
if IMPORTS_SUCCESS:
    try:
        ...
        print(f"Service initialization error: {e}", file=sys.stderr)
        gemini_service = None
...
def load_regulations():
    ...
    paths_to_try = [
        ...
        Path('/var/task/api/data/regulations.json'),
    ]
...
async def root():
    ...
    return {
        ...
        "services_initialized": all([gemini_service, matching_engine, firebase_service]),
        "endpoints": {
            "health": "/api/health",
...
async def health_check():
    ...
    return {
        ...
        "environment": {
            ...
            "FIREBASE_CONFIG": bool(os.getenv('FIREBASE_SERVICE_ACCOUNT')),
        }
    }
...
async def get_regulations():
    ...
    try:
        if not regulations_data:
            return {
                "success": False,
                "message": "Regulations data not loaded",
                "regulations": []
            }
